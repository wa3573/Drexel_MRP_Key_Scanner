...
vector<pair<int, string> > MidiInputController::availableMidiDevices() {
	...
	try {
        std::vector<std::string> deviceStrings = MidiInput::getDevices();
		
		for(int i = 0; i < (int) deviceStrings.size(); i++) {
...
//		if(it->second != string(kMidiVirtualOutputName))
...
std::string MidiInputController::deviceName(int portNumber) {
    std::vector<std::string> const& deviceStrings = MidiInput::getDevices();
    if(portNumber < 0 || portNumber >= (int) deviceStrings.size())
        return "";
    return deviceStrings[portNumber];
}

// Find the index of a device with a given name; return -1 if not found
int MidiInputController::indexOfDeviceNamed(std::string const& name) {
    std::vector<std::string> const& deviceStrings = MidiInput::getDevices();
...
OscMessage* MidiInputController::oscControlMessageForSegment(int segment, const char *path, const char *types,
    ...
    ScopedLock sl(segmentsMutex_);
    if(segment < 0 || segment >= (int) segments_.size())
        return 0;
