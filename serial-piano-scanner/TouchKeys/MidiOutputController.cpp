...
vector<pair<int, string> > MidiOutputController::availableMidiDevices()
{
	...
	try {
		std::vector<std::string> deviceStrings = MidiOutput::getDevices();

		for (int i = 0; i < deviceStrings.size(); i++) {
			pair<int, string> p(i, string(deviceStrings[i]));
...
std::string MidiOutputController::deviceName(int portNumber)
{
	std::vector<std::string> deviceStrings = MidiOutput::getDevices();
...
int MidiOutputController::indexOfDeviceNamed(std::string const& name)
{
	std::vector<std::string> deviceStrings = MidiOutput::getDevices();
...
void MidiOutputController::sendNoteOn(int port, unsigned char channel,
...
//                            (int)(note & 0x7F),
//                            (int)(velocity & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessageNoteOn, note, velocity));
}

// Send a MIDI Note Off message
void MidiOutputController::sendNoteOff(int port, unsigned char channel,
		unsigned char note, unsigned char velocity)
{
//	sendMessage(port,
//			MidiMessage((int) ((channel & 0x0F) | kMidiMessageNoteOff),
//					(int) (note & 0x7F), (int) (velocity & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessageNoteOff, note, velocity));

}

// Send a MIDI Control Change message
void MidiOutputController::sendControlChange(int port, unsigned char channel,
		unsigned char control, unsigned char value)
{
//	sendMessage(port,
//			MidiMessage((int) ((channel & 0x0F) | kMidiMessageControlChange),
//					(int) (control & 0x7F), (int) (value & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessageControlChange, control, value));

}

// Send a MIDI Program Change message
void MidiOutputController::sendProgramChange(int port, unsigned char channel,
		unsigned char value)
{
//	sendMessage(port,
//			MidiMessage((int) ((channel & 0x0F) | kMidiMessageProgramChange),
//					(int) (value & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessageProgramChange, 0, value));

}

// Send a Channel Aftertouch message
void MidiOutputController::sendAftertouchChannel(int port,
		unsigned char channel, unsigned char value)
{
//	sendMessage(port,
//			MidiMessage(
//					(int) ((channel & 0x0F) | kMidiMessageAftertouchChannel),
//					(int) (value & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessageAftertouchChannel, 0, value));
}

// Send a Polyphonic Aftertouch message
void MidiOutputController::sendAftertouchPoly(int port, unsigned char channel,
		unsigned char note, unsigned char value)
{
//	sendMessage(port,
//			MidiMessage((int) ((channel & 0x0F) | kMidiMessageAftertouchPoly),
//					(int) (note & 0x7F), (int) (value & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessageAftertouchPoly, note, value));
}

// Send a Pitch Wheel message
void MidiOutputController::sendPitchWheel(int port, unsigned char channel,
		unsigned int value)
{
//	sendMessage(port,
//			MidiMessage((int) ((channel & 0x0F) | kMidiMessagePitchWheel),
//					(int) (value & 0x7F), (int) ((value >> 7) & 0x7F)));
	sendMessage(port, MidiMessage(channel, kMidiMessagePitchWheel, 0, value));
}
...
//	sendMessage(port, MidiMessage(kMidiMessageReset));
	sendMessage(port, MidiMessage(0, kMidiMessageReset, 0, 0));
