...
// Default constructor
KeyIdleDetector::KeyIdleDetector(capacity_type capacity,
		Node<key_position>& keyBuffer, key_position positionThreshold,
		key_position activityThreshold, int counterThreshold) :
		Node<int>(capacity), keyBuffer_(keyBuffer), accumulator_(
				kKeyIdleNumSamples + 1, keyBuffer), keyIdleThreshold_(
				kDefaultKeyIdleThreshold), activityThreshold_(
				activityThreshold), positionThreshold_(positionThreshold), numberOfFramesWithoutActivity_(
				0), noActivityCounterThreshold_(counterThreshold), idleState_(
				kIdleDetectorUnknown)
{
...
void KeyIdleDetector::triggerReceived(TriggerSource* who,
		timestamp_type timestamp)
{
	...
	if (idleState_ == kIdleDetectorIdle) {
		...
		key_position averageValue = currentAccumulator.second
				/ (key_position) currentAccumulator.first;
