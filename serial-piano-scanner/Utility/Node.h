...
struct NodeReverseIterator : public boost::reverse_iterator<T> {
	...
	size_type index() const { return boost::fusion::prior(this->base_reference()).index(); }
	timestamp_type timestamp() const { return boost::fusion::prior(this->base_reference()).timestamp(); }
};
