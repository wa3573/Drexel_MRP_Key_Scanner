...
//            std::string presetFile = applicationProperties_.getUserSettings()->getValue("LastSavedPreset");
//            if(presetFile != "") {
...
//        std::string presetFile = getPrefsStartupPreset();
//        if(presetFile != "") {
//            loadPresetFromFile(presetFile.toUTF8());
//        }
//    }
...
//        std::string tkDevicePath = applicationProperties_.getUserSettings()->getValue("TouchKeysDevice");
...
//				//Logger::writeToLog(std::string::formatted("Found " + comPortName));
...
//					//Logger::writeToLog(std::string("2Didn't find " + comPortName + "; error " + std::string(error)));
//				}
//				delete [] lpCC;
//			}
//			else {
//				int error = GetLastError();
//				//Logger::writeToLog(std::string("Didn't find " + comPortName + "; error " + std::string(error)));
//			}
//		}
//    }
//#else
...
//    if(success)
//        applicationProperties_.getUserSettings()->setValue("TouchKeysDevice", std::string(path));
    return success;
}
...
void MainApplicationController::enableMIDIOutputVirtualPort(int identifier, const char *name) {
    ...
    std::string zoneName = "MIDIOutputZone";
    zoneName += identifier;
    std::string zoneValue = "__virtual__";
    zoneValue += std::string(name);
...
int MainApplicationController::oscTransmitAddAddress(const char * host, const char * port, int proto) {
    ...
    if(indexOfNewAddress >= 0) {
        ...
        std::string keyName = "OSCTransmitHost";
        keyName += indexOfNewAddress;
//        applicationProperties_.getUserSettings()->setValue(keyName, std::string(host));
        
        keyName = "OSCTransmitPort";
        keyName += indexOfNewAddress;
//        applicationProperties_.getUserSettings()->setValue(keyName, std::string(port));
...
std::string MainApplicationController::getPrefsStartupPreset() {
//    if(!applicationProperties_.getUserSettings()->containsKey("StartupPreset"))
//        return "";
...
//        std::string deviceName = props->getValue("MIDIInputPrimary");
...
//        std::string deviceName = props->getValue("MIDIInputAuxiliary");
//        int index = midiInputController_.indexOfDeviceNamed(deviceName);
...
//    PropertiesFile *props = applicationProperties_.getUserSettings();
//
//    std::string keyName = "MIDIOutputZone";
//    keyName += zone;
//
//    if(props->containsKey(keyName)) {
//        std::string output = props->getValue(keyName);
//        if(output.startsWith("__virtual__")) {
//#ifndef JUCE_WINDOWS
//            // Open virtual port with the name that follows
//            std::string virtualPortName = output.substring(11); // length of "__virtual__"
//            midiOutputController_.enableVirtualPort(zone, virtualPortName.toUTF8());
//#endif
//        }
//        else {
//            std::string deviceName = props->getValue(keyName);
